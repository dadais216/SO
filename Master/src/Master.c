...
void establecerConexiones() {
	...
	socketWorker = socketCrearCliente(configuracion->ipWorker, configuracion->puertoWorker, ID_MASTER),	imprimirMensaje(archivoLog, "[CONEXION] Conexion existosa con Worker");
}
...
WorkerReduccion* deserializarReduccion(Mensaje* mensaje) {
	...
	memcpy(&nombretemporalreduccion,
			mensaje->datos + 20 + sizeof(int) * 2 + tamaniolista, 12);
...
void establecerConexionConWorker(Lista bloques) {
	...
	int i;
	enviarBloques: for (i = 0; i < bloques->elements_count; i++) {
